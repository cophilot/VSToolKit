...
//workspace path
const workspacePath = path.join(__dirname, '../', 'vstoolkit-workspace');
    ...
    vscode.window
        ...
        .then(async (value) => {
            let terminal = vscode.window.createTerminal({
                name: 'VSToolKit install dependencies',
                hideFromUser: false,
            });
...
function activate(context) {
    ...
    vscode.commands.registerCommand(
        ...
        async function () {
            ...
            if (text.charAt(0).toUpperCase() == text.charAt(0)) {
                ...
                translatedText =
                    translatedText.charAt(0).toLowerCase() +
                    translatedText.slice(1);
            }
    ...
    vscode.commands.registerCommand(
        ...
        async function () {
            ...
            let settings =
                vscode.workspace.getConfiguration('vstoolkit.snapshot');
            //create snapshot
    ...
    vscode.commands.registerCommand(
        'vstoolkit.pasteToAdvancedClipboard',
        async function () {
